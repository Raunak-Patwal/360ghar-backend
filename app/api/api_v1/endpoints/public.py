...
def _is_ip_rate_limited(
    ...
    *,
    ...
    last_reap_raw = store.get(last_reap_attr)
    last_reap: float = last_reap_raw[0] if last_reap_raw else 0.0
    if now - last_reap > _REAP_INTERVAL_S:
        with _reaper_lock:
            # Re-check under lock so two workers don't both decide to reap
            current_raw = store.get(last_reap_attr)
            current_reap: float = current_raw[0] if current_raw else 0.0
