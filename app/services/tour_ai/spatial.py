...
def _best_opening_for(scene: dict[str, Any], target_room_type: str) -> dict[str, Any] | None:
    ...
    if not pool:
        return None
    import typing
    return typing.cast(dict[str, typing.Any], max(pool, key=lambda o: o.get("leads_to_confidence", 0.0)))
...
def start_scene_id(scenes: list[dict[str, Any]]) -> str:
    ...
    return str(ranked[0]["id"]) if ranked else str(scenes[0]["id"])
...
def _fallback_opening(scene: dict[str, Any]) -> dict[str, Any]:
    if scene["openings"]:
        import typing
        return typing.cast(dict[str, typing.Any], scene["openings"][0])
